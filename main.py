...
def load_images():
    ...
    for piece in pieces:
        ...
        IMAGES[piece] = p.image.load(os.path.join(image_path, f'{piece}.png')).convert_alpha()
...
def main():
    ...
    move_sound = p.mixer.Sound(os.path.join(sound_path, 'move.wav'))
    capture_sound = p.mixer.Sound(os.path.join(sound_path, 'capture.wav'))
    ...
    while running:
        ...
        white_vs_ai_button = Button(pos=(336, 400), text_input="White v/s AI", font=p.font.SysFont(
            "Helvetica", 32, True, False), base_color=p.Color("White"), hovering_color=p.Color("Grey"))

        white_vs_ai_button.changeColor(menu_mouse_pos)
        white_vs_ai_button.update(screen)

        black_vs_ai_button = Button(pos=(336, 475), text_input="Black v/s AI", font=p.font.SysFont(
            "Helvetica", 32, True, False), base_color=p.Color("White"), hovering_color=p.Color("Grey"))

        black_vs_ai_button.changeColor(menu_mouse_pos)
        black_vs_ai_button.update(screen)

        play_friend = Button(pos=(336, 325), text_input="P v/s P", font=p.font.SysFont(
            "Helvetica", 32, True, False), base_color=p.Color("White"), hovering_color=p.Color("Grey"))

        play_friend.changeColor(menu_mouse_pos)
        play_friend.update(screen)

        human_turn = (gs.white_to_move and white_human) or (
            not gs.white_to_move and black_human)
        ...
        if not game_over and not human_turn and not move_undone:
            ...
            if not ai_thinking:
                ...
                move_finder_process = multiprocessing.Process(
                    target=ChessBrain.find_best_move, args=(gs, valid_moves, return_queue))
            ...
            if not move_finder_process.is_alive():
                ...
                if not (not white_human and not black_human and len(gs.movelog) == 0):
        ...
        if gs.is_stalemate:
            ...
            screen.blit(s, (0, 0))
            draw_text(screen, "Stalemate", p.Color("White"))
...
def highlight_squares(screen, gs, valid_moves, sq_selected):
    ...
    if len(gs.movelog) != 0:
        screen.blit(s, (abs(gs.movelog[-1].start_col - flipped) *
                    SQ_SIZE, abs(gs.movelog[-1].start_row - flipped) * SQ_SIZE))
        screen.blit(s, (abs(gs.movelog[-1].end_col - flipped) *
                    SQ_SIZE, abs(gs.movelog[-1].end_row - flipped) * SQ_SIZE))

        if gs.in_check:
            s.fill(p.Color("red"))
            if gs.white_to_move:
                screen.blit(
                    s, (abs(gs.white_king_location[1] - flipped) * SQ_SIZE, abs(gs.white_king_location[0] - flipped) * SQ_SIZE))
            else:
                screen.blit(
                    s, (abs(gs.black_king_location[1] - flipped) * SQ_SIZE, abs(gs.black_king_location[0] - flipped) * SQ_SIZE))
...
def draw_board(screen):
    ...
    global colors
    colors = [p.Color("White"), p.Color("#4b739a")]
